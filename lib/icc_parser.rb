...
class IccParser
  ...
  def self.parse_string(icc_profile)
    ...
    ret[:date] = 
      if icc_profile[24..35].unpack('n*').first > 0
        DateTime.new(*icc_profile[24..35].unpack('n*')) # ok
      else
        nil
      end
    ...
    ret[:flags] = icc_profile[44..47].unpack('L>') # flags could actually be interpreted
    ret[:manufacturer] = icc_profile[40..43].first.encode('utf-8') # ok
      ...
      value =
        ...
        when 'cprt'
          begin
            ...
            icc_profile[offset + 8..offset + size - 2].strip
          end
        when 'desc'
